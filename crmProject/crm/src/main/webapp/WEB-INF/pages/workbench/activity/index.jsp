    ...
    <script type="text/javascript">
        ...
        $(function () {
            ...
            $("#create-marketActivityName").blur(function () {
                if ($.trim($("#create-marketActivityName").val()) == "") {
                    $("#nameMsg").text("名称不能为空")
                } else {
                    $("#nameMsg").text("")
                }
...
/*            $("#tBody input[type='checkbox']").click(function () {
            ...
            $("#importActivityBtn").click(function () {
                ...
                activityFileName = activityFileName.substr(activityFileName.lastIndexOf(".")+1).toUpperCase()
                //表单验证
                //xls Xls XLS xLS .....
                if (activityFileName!="XLS"){
                    alert("文件名扩展名必须是.xls")
                    return;
                }
                ...
                $.ajax({
                    ...
                    type:"post",
                    dataType:"json",
                    //默认情况下ajax发送请求,会把所有携带参数转换为字符串
                    processData:false,
                    //默认情况下,ajax发送请求会将所有的参数编码统一进行urlencoded编码
                    contentType:false,
                    success:function (data) {
                        if (data.code=="1"){
                            //提示导入条数
        ...
        function queryActivityForPageByCondition(pageNo, pageSize) {
            ...
            //发送请求
            $.ajax({
                url: "workbench/activity/queryActivityForPageByCondition",
                data: {
                    pageNo: pageNo,
                    pageSize: pageSize,
                    name: name,
